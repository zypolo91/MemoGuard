...
import { fetchMemories as fetchMemoriesApi } from "@/services/api/memories";
...
const POSITIVE_WORDS = ["ÎÂÅ¯", "¿ªÐÄ", "ÐÀÎ¿", "ÇáËÉ", "¸Ð¶¯", "Óä¿ì", "¾ªÏ²", "Âú×ã", "Ç×ÃÜ"];
const NEGATIVE_WORDS = ["µ£ÐÄ", "½¹ÂÇ", "Æ£±¹", "ÒÅº¶", "Ê§Âä", "ÄÑ¹ý", "¾ÚÉ¥"];
...
function createSummary(text: string): string {
  ...
  if (!plain) return "ÉÐÎ´×«Ð´ÏêÏ¸ÄÚÈÝ";
...
function normalizeMemory(raw: any): MemoryItem {
  ...
  return {
    ...
    title: String(raw.title ?? "Î´ÃüÃû»ØÒä"),
...
export const useMemoriesStore = defineStore("memories", () => {
  ...
  async function fetchMemories() {
    ...
    error.value = null;
    try {
      const remote = await fetchMemoriesApi();
