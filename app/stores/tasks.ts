...
import { fetchTasks as fetchTasksApi } from "@/services/api/tasks";
...
function normalizeTask(raw: any): CareTask {
  ...
  return {
    id: String(raw.id ?? `t-${Math.random().toString(36).slice(2, 10)}`),
    title: String(raw.title ?? "未命名任务"),
    category: String(raw.category ?? "护理"),
    frequency: raw.frequency ?? "once",
    startAt: String(raw.dueAt ?? raw.startAt ?? baseTimestamp),
    endAt: raw.endAt ?? null,
    priority: (raw.priority ?? "medium") as "low" | "medium" | "high",
    reminderLead: Number(raw.reminderLead ?? 15),
    reminderChannel: Array.isArray(raw.reminderChannel) ? raw.reminderChannel : ["app"],
    notes: typeof raw.description === "string" ? raw.description : raw.notes ?? "",
    statusHistory: history.map((entry: any) => ({
      status: (entry.status ?? "pending") as TaskStatus,
      timestamp: String(entry.timestamp ?? baseTimestamp)
    }))
  };
}
export const useTasksStore = defineStore("tasks", () => {
  ...
  async function fetchTasks() {
    ...
    error.value = null;
    try {
      tasks.value = (await fetchTasksApi()).map((item) => normalizeTask(item));
      state.value = "success";
    } catch (err) {
